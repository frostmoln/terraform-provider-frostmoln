...
// `tags = data.frostmoln_public_ip.x.tags` into any network write would 400 on the
// reserved key. `tags` has to mean customer tags on every surface.
func TestPublicIPDataSource_ReservedTagsNeverReachState(t *testing.T) {
	ctx := context.Background()
	var d publicIPDataSource
	...
	d.setState(ctx, &state, &apiPublicIP{
		ID: "fip-1",
