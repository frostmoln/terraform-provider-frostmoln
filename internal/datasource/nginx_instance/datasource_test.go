...
func configVal(t *testing.T, id string) tftypes.Value {
	...
	return tftypes.NewValue(tfType, map[string]tftypes.Value{
		...
		"worker_processes": tftypes.NewValue(tftypes.Number, nil),
...
func TestReadByID(t *testing.T) {
	server := newTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodGet && r.URL.Path == "/v1/tenants/tenant-1/webservers/ws-1" {
			...
			_ = json.NewEncoder(w).Encode(apiWebserverInstance{
				...
				EngineConfig:    "client_max_body_size 10m;",
				Status:          "running",
				PrivateIP:       "10.0.1.6",
				Port:            443,
	...
	if state.WorkerProcesses.ValueInt64() != 4 {
		t.Errorf("expected WorkerProcesses 4, got %d", state.WorkerProcesses.ValueInt64())
	}
	if state.TryFiles.ValueString() != "$uri $uri/ =404" {
		t.Errorf("expected TryFiles, got %s", state.TryFiles.ValueString())
	}
	if state.ProxyPass.ValueString() != "http://localhost:3000" {
		t.Errorf("expected ProxyPass, got %s", state.ProxyPass.ValueString())
	}
...
func TestReadByIDNullableFieldsEmpty(t *testing.T) {
	...
	if !state.WorkerProcesses.IsNull() {
		t.Error("expected null worker_processes")
	}
	if !state.TryFiles.IsNull() {
