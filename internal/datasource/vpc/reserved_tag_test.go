...
// `tags = data.frostmoln_vpc.x.tags` into any network write would 400 on the
// reserved key. `tags` has to mean customer tags on every surface.
func TestVPCDataSource_ReservedTagsNeverReachState(t *testing.T) {
	...
	d.setVPCState(ctx, &state, &apiVPC{
		ID: "vpc-1",
