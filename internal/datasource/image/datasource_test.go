...
// TestImageLookupIsTenantScoped is the regression guard for the data source's
// one API call.
//
// The api-gateway's EffectiveTenant middleware re-scopes and re-signs the
// auth-context JWT only for paths matching /tenants/{id}/. Read from the bare
...
// tenant_id selector does, so a bare-path regression reads the wrong tenant and
// is caught here.
func TestImageLookupIsTenantScoped(t *testing.T) {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, req *http.Request) {
		...
		case strings.HasSuffix(req.URL.Path, "/images"):
			_ = json.NewEncoder(w).Encode(apiImageList{Images: []apiImage{
				{ID: "img-1", Name: "Ubuntu 22.04", Status: "active", CreatedAt: "2025-01-01T00:00:00Z"},
			}})
		default:
	...
	if !slices.Contains(paths, want) {
		t.Errorf("the image lookup requested %v; it must name the configured tenant (%s). "+
