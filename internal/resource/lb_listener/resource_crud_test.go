...
func fastListenerDeleteResource(t *testing.T, c *client.Client) *listenerResource {
	t.Helper()
	r := &listenerResource{client: c}
...
// TestDeleteWaitsForTheDeleteOperation: a listener delete routes through
...
// when the 202 lands — the listener may still be serving behind that envelope.
func TestDeleteWaitsForTheDeleteOperation(t *testing.T) {
	polled := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodDelete && r.URL.Path == "/v1/tenants/t-1/load-balancers/lb-1/listeners/l-1":
			w.WriteHeader(http.StatusAccepted)
			_ = json.NewEncoder(w).Encode(map[string]any{
				"operationId": "op-del", "status": "pending", "resourceType": "listener",
			})
		case r.Method == http.MethodGet && r.URL.Path == "/v1/tenants/t-1/operations/op-del":
			polled++
			_ = json.NewEncoder(w).Encode(map[string]any{
				"operationId": "op-del", "status": "completed", "resourceType": "listener",
			})
		default:
			w.WriteHeader(http.StatusNotFound)
	...
	c := client.NewClient(server.URL, "test-key", client.WithHTTPClient(server.Client())) // pragma: allowlist secret
	c.SetTenantIDForTest("t-1")
	r := fastListenerDeleteResource(t, c)
...
// the listener still exists and the state row must stay, with the refusal
...
func TestDeleteOperationFailureRefusesAndKeepsState(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v1/tenants/t-1/operations/op-del":
			_ = json.NewEncoder(w).Encode(map[string]any{
				"operationId": "op-del", "status": "failed", "resourceType": "listener",
				"error": "listener still holds active pool members",
			})
		default:
			w.WriteHeader(http.StatusNotFound)
			_ = json.NewEncoder(w).Encode(map[string]string{"code": "NOT_FOUND", "message": "not found"})
	...
	if !strings.Contains(err.Detail(), "listener still holds active pool members") {
