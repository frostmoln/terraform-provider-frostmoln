...
// store and echo it while the running Redis keeps its previous behaviour
...
func TestUpdateRefusesEvictionPolicyChange(t *testing.T) {
	...
	if got, want := err.Summary(), "eviction_policy cannot be changed on a running Redis instance"; got != want {
...
func TestUpdateRefusesPersistenceModeChange(t *testing.T) {
	...
	if got, want := updateResp.Diagnostics.Errors()[0].Summary(), "persistence_mode cannot be changed on a running Redis instance"; got != want {
		t.Errorf("expected summary %q, got %q", want, got)
	}
	if requested.Load() {
		t.Error("the refusal must fire before any API request")
...
func TestModifyPlanWarnsCreateRenderedChange(t *testing.T) {
	...
	if got, want := warnings[0].Summary(), "eviction_policy cannot be changed on a running Redis instance"; got != want {
...
func TestModifyPlanCreateAndDestroyStayClean(t *testing.T) {
	...
	createModel := RedisInstanceModel{
		ID:              types.StringValue("redis-123"),
	...
	resp := resource.ModifyPlanResponse{}
	r.ModifyPlan(ctx, resource.ModifyPlanRequest{State: emptyState, Plan: buildRedisInstancePlan(t, createModel)}, &resp)
	...
	populated := buildRedisInstanceState(t, createModel)
	resp = resource.ModifyPlanResponse{}
...
// A storage_gb increase routes through POST /caches/{id}/resize (grow-only), NOT the PUT (which
// the backend ignores for storage), and skips the PUT when nothing else changed.
func TestUpdateStorageResize(t *testing.T) {
	...
	base := RedisInstanceModel{
		...
		FlavorID: types.StringValue("cache.small"), VPCID: types.StringValue("vpc-1"), SubnetID: types.StringValue("sn-1"),
...
func TestUpdateStorageResizeWaitsForTheOperation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/v1/tenants/t-1/caches/redis-1/resize":
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v1/tenants/t-1/caches/redis-1":
			_ = json.NewEncoder(w).Encode(apiRedisInstance{ID: "redis-1", Name: "redis", Status: "running", StorageGB: 40})
		default:
			w.WriteHeader(http.StatusNotFound)
	...
	c.SetTenantIDForTest("t-1")
	r := &redisInstanceResource{client: c, pollInterval: 5 * time.Millisecond}

	resp, err := r.client.Post(context.Background(), r.client.TenantPath("/caches/redis-1/resize"), apiResizeRedisInstanceRequest{StorageGB: 40})
	if err != nil {
		t.Fatalf("resize POST failed: %v", err)
	}
	if err := r.awaitResize(context.Background(), "redis-1", resp, 200*time.Millisecond); err != nil {
...
func TestUpdateStorageResizeOperationRefusedSurfaces(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v1/tenants/t-1/caches/redis-1":
			_ = json.NewEncoder(w).Encode(apiRedisInstance{ID: "redis-1", Name: "redis", Status: "running", StorageGB: 20})
		default:
			w.WriteHeader(http.StatusNotFound)
			_ = json.NewEncoder(w).Encode(map[string]string{"code": "NOT_FOUND", "message": "not found"})
		}
	...
	err = r.awaitResize(context.Background(), "redis-1", resp, 200*time.Millisecond)
	if err == nil {
