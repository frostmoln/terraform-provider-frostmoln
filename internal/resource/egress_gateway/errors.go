...
//
//   - EGRESS_MODE_UNAVAILABLE is a 400, which reads as "fix your syntax". It is
...
func addEgressError(diags *diag.Diagnostics, fallback string, err error) {
	...
	case errCodeModeUnavailable:
		diags.AddError(
			"Egress gateway mode is not available",
	...
	case errCodeGatewayInUse:
		diags.AddError(
			...
			"Something in this VPC still depends on the egress gateway — an address associated here "+
				...
				"gateway create then fails with EGRESS_GATEWAY_EXISTS. A `depends_on` written the "+
