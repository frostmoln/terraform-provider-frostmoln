...
func TestUpdateStorageResizeWaitsForTheOperation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/v1/tenants/t-1/caches/valkey-1/resize":
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v1/tenants/t-1/operations/op-resize":
			...
			_ = json.NewEncoder(w).Encode(map[string]any{
				"operationId": "op-resize", "status": "completed", "resourceType": "cache",
			})
		case r.Method == http.MethodGet && r.URL.Path == "/v1/tenants/t-1/caches/valkey-1":
			_ = json.NewEncoder(w).Encode(apiValkeyInstance{ID: "valkey-1", Name: "valkey", Status: "running", StorageGB: 40})
		default:
			w.WriteHeader(http.StatusNotFound)
	...
	c := client.NewClient(server.URL, "test-key", client.WithHTTPClient(server.Client())) // pragma: allowlist secret
	c.SetTenantIDForTest("t-1")
	r := &valkeyInstanceResource{client: c, pollInterval: 5 * time.Millisecond}

	resp, err := r.client.Post(context.Background(), r.client.TenantPath("/caches/valkey-1/resize"), apiResizeValkeyInstanceRequest{StorageGB: 40})
	if err != nil {
		t.Fatalf("resize POST failed: %v", err)
	}
	if err := r.awaitResize(context.Background(), "valkey-1", resp, 200*time.Millisecond); err != nil {
...
func TestUpdateStorageResizeOperationRefusedSurfaces(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v1/tenants/t-1/operations/op-resize":
			_ = json.NewEncoder(w).Encode(map[string]any{
				"operationId": "op-resize", "status": "failed", "resourceType": "cache",
				"errorCode": "invalid", "error": "storage can only grow",
			})
		case r.Method == http.MethodGet && r.URL.Path == "/v1/tenants/t-1/caches/valkey-1":
			_ = json.NewEncoder(w).Encode(apiValkeyInstance{ID: "valkey-1", Name: "valkey", Status: "running", StorageGB: 20})
		default:
			w.WriteHeader(http.StatusNotFound)
			_ = json.NewEncoder(w).Encode(map[string]string{"code": "NOT_FOUND", "message": "not found"})
		}
	...
	err = r.awaitResize(context.Background(), "valkey-1", resp, 200*time.Millisecond)
	if err == nil {
...
// --- create-wait orphan contract (internal/orphan): the wait's failure is
// classified, and the unresolved arm sweeps the family listing to resolve what
// this apply produced rather than orphaning it. Mirrors the redis tests. ---
...
func TestCreateWaitTimeoutAdopts(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case p == "/v1/tenants/t-1/caches":
			listed = true
			_ = json.NewEncoder(w).Encode(apiValkeyInstanceList{Instances: []apiInstanceRef{
				{ID: "valkey-fresh", Name: "test-valkey", Type: "valkey", CreatedAt: now},
				// A same-named REDIS sibling must never be adopted.
				{ID: "redis-fresh", Name: "test-valkey", Type: "redis", CreatedAt: now},
				// An older same-type instance predates this apply.
				{ID: "valkey-stale", Name: "test-valkey", Type: "valkey", CreatedAt: "2020-01-01T00:00:00Z"},
			}})
		case p == "/v1/tenants/t-1/caches/valkey-fresh":
			// The adoption's honest read: the platform's row, running.
			_ = json.NewEncoder(w).Encode(apiValkeyInstance{
				ID: "valkey-fresh", Name: "test-valkey", TypeVersion: "7.2",
	...
	c := client.NewClient(server.URL, "test-key", client.WithHTTPClient(server.Client())) // pragma: allowlist secret
	c.SetTenantIDForTest("t-1")
	r := &valkeyInstanceResource{client: c, pollInterval: 10 * time.Millisecond, pollTimeout: 300 * time.Millisecond}
	...
	if result.ID.ValueString() != "valkey-fresh" {
		t.Errorf("adopted id = %q, want valkey-fresh (the apply's own instance)", result.ID.ValueString())
	}
	if result.Status.ValueString() != "running" {
...
func TestCreateWaitTimeoutVerifiedAbsent(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case p == "/v1/tenants/t-1/caches":
			_ = json.NewEncoder(w).Encode(apiValkeyInstanceList{})
		case strings.HasSuffix(p, "/events"):
			w.WriteHeader(http.StatusNotFound)
		default:
			t.Errorf("unexpected request: %s %s", r.Method, p)
