...
**Tenant default tags.** The provider's `default_tags` apply only to what this provider configuration writes. A tenant can also have default tags of its own — set in the portal, with the `fm` CLI, or with the `frostmoln_tenant_default_tags` resource — which the platform copies onto every taggable resource created in the tenant, whoever creates it. On a Terraform-managed resource they appear in `tags_all`, never in `tags`, and are kept like any other key set outside Terraform. A key the resource's `tags` or the provider's `default_tags` sets wins over a tenant default of the same key. They are applied only when a resource is created: changing them does not change existing resources.
