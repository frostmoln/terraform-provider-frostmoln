...
description: |-
  ...
  Create-immutable — description, direction, port_range_max, port_range_min, protocol, remote_cidr, remote_group_id, security_group_id: a rule has no in-place update — the platform registers POST and DELETE and nothing else — so a change destroys and re-creates it.
---
